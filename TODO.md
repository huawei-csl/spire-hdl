...
- ~~probabliy not a nice pattern: type(elem).wire_like(elem), better do -> elem.get_wire_clone()~~ (done: `get_wire_clone()` is generic on `HDLComposite`; `wire_like` is a shim, subclasses override only where construction differs)
