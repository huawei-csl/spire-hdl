...
- Update documentation comprehensively. Later patches still leave references to `CompositeRecordDynamic`, `record_dynamic.py`, and old record construction styles in user-facing docs such as `README.md` and `docs/README_composite_types.md`. The docs should describe the unified `CompositeRecord`, the dynamic `super().__init__(...)` style, dataclass support, and any restored class-template style once implemented.
