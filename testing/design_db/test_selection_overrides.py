...
Both doors force ``select_design`` (and therefore ``@from_design_db`` splices) to specific
design_ids without touching source — and are never recorded in the manifest.
"""
import json
import os
import subprocess
...
from spire.design_db import (DesignDBError, PINS_ENV, from_design_db, insert_design,
                             register_slot, seed_original, select_design, selection_overrides)
...
def _two_design_slot():
    ...
    argmin = select_design(key, objective="area").design_id
    other = b if argmin == a else a
    return key, argmin, other


def test_env_pins_force_selection_and_never_record(db, monkeypatch):
    ...
    assert sel.design_id == other                       # override beat the argmin
    man = DesignDB.open(db).read_json(DesignDB.open(db).manifest_path)
    assert man["slots"]["adder8"].get("selected_id") is None or \
        "selected_id" not in man["slots"]["adder8"]     # what-if compile: nothing recorded
    monkeypatch.delenv(PINS_ENV)
    assert select_design(key, objective="area").design_id == argmin   # back to normal


def test_context_manager_scoped_and_nested(db):
    key, argmin, other = _two_design_slot()
    with selection_overrides({key: other}):
        assert select_design(key).design_id == other
        with selection_overrides({key: argmin}):        # innermost wins
            assert select_design(key).design_id == argmin
        assert select_design(key).design_id == other
    assert select_design(key).design_id == argmin       # scope ended
...
def test_explicit_pin_wins_over_override(db):
    key, argmin, other = _two_design_slot()
    with selection_overrides({key: other}):
        assert select_design(key, pin=argmin).design_id == argmin
...
def test_env_pins_cross_process(db, monkeypatch):
    ...
    key, argmin, other = _two_design_slot()
    code = ("from spire.design_db import select_design; "
            f"print(select_design({key!r}, objective='area').design_id)")
