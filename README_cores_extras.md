...
python -m spirehdl.arithmetic.arithmetic_generator adder \
  --n-bits 16 \
...
python -m spirehdl.arithmetic.arithmetic_generator matmulacc \
...
python -m spirehdl.arithmetic.arithmetic_generator matmulacc-fused \
  --dim-m 4 --dim-n 4 --dim-k 4 \
  --a-width 8 \
  --ppg-opt BAUGH_WOOLEY \
  --ppa-opt WALLACE_TREE \
...
python -m spirehdl.arithmetic.arithmetic_generator fpmatmulacc \
  --dim-m 4 --dim-n 4 --dim-k 4 \
...
```bash
python -m spirehdl.arithmetic.int_multipliers.eval.run_multiplier_stage_options_eval_ext_stat
```
This will generate a parquet file in the folder `data`. Visualization can be done with the plotly app via 
```bash
python -m spirehdl.arithmetic.int_multipliers.eval.plot.plotly_app --file data/data_file.parquet
``` 
or with the script 
```bash
python -m spirehdl.arithmetic.int_multipliers.eval.plot.multiplier_stage_plot --file data/data_file.parquet
```
Replace `data_file.parquet` with the file produced by the evaluate script.

If desired, new multiplier options can be added here: `spirehdl/arithmetic/int_multipliers/eval/multiplier_stage_options_demo_lib.py`.

#### Optimized Multipliers
The multipliers in `spirehdl/arithmetic/int_multipliers/multipliers/multipliers_ext_optimized.py` rely on precomputed AIGs (and map files). By default they load packaged assets from `spirehdl/arithmetic/int_multipliers/data/optimized/` using the following filenames:
