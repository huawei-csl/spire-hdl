"""Post-construction structural CSE (Common Subexpression Elimination) for SproutHDL modules.
...
1. **High fan-out on a single Expr instance.** `a & b` constructed once may be reused by N parents.
   ``_maybe_share`` in ``sprouthdl.py`` catches this via ``as_expr``, but only on operands that go
...
the wire. The result is information-preserving across the SproutHDL → Verilog boundary: the emitted
Verilog's AIG gate count exactly matches the sprouthdl-native AigerExporter count.
"""
