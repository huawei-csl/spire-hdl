...
from spirehdl.aig.aig_to_aag import aig_file_to_aag_lines
from spirehdl.aig.aig_yosys import verilog_to_aag_lines_via_yosys, verilog_to_aag_via_yosys
from spirehdl.aig.aig_yosys import aig_file_to_aag_lines_via_yosys

# Package data locations for precomputed multipliers
RESOURCE_PACKAGE = "spirehdl.arithmetic.int_multipliers.data"
...
def _load_aag_lines_from_resources(aig_rel: Path, map_rel: Path, desc: str) -> List[str]:
    ...
    if missing:
        raise FileNotFoundError(
            ...
            "Add the files under 'src/spirehdl/arithmetic/int_multipliers/data/optimized/' "
...
def get_aag_lines_default(aw: int, bw: int, a_encoding: Encoding, b_encoding: Encoding) -> List[str]:
    ...
    if root is None or aw != bw:
        raise NotImplementedError(
            ...
            "Provide 'f_aag_lines' or add the expected AIG files under 'spirehdl/arithmetic/int_multipliers/data/'."
        )
...
def test_multiplier_ext_optimized() -> None:
    ...
    from spirehdl.arithmetic.int_multipliers.eval.multiplier_stage_options_demo_lib import MultiplierOption, encoding_for_multiplier
