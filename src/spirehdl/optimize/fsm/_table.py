"""Step 6: extract an FSM transition table from a register's driver tree.

Given the FSM's state register ``reg`` (whose ``_driver`` is the full
next-state mux tree the user built inside the ``with`` block) and optionally
a list of Moore output signals, enumerate every
``(state_value × input_combination)`` pair and evaluate the driver to recover
``next_state_value`` and per-output values.

This is the "symbolic eval over an Expr DAG" use of ``_evaluator``. The
inputs are auto-discovered via ``_walker.find_input_signals`` (every Signal
leaf reachable from the driver, minus ``reg`` itself and minus auto-shared
CSE wires).
"""
...
# Conservative cap on the per-FSM input domain. 65 536 = 2^16 input bits, which
# covers any FSM with a few 1-bit / few-bit inputs. Larger needs the future
# symbolic-input variant — for now we skip minimisation on those.
...
class TransitionTable:
    ...
    ``input_signals`` records the (deduped) list of free input Signals in
    the order their values appear in ``input_tuple``. ``input_combos`` is the
    explicit list of input tuples enumerated — derived from ``input_signals``
    by the extractor but can be set directly by tests / synthetic callers.
    """
...
def extract_transition_table(
    ...
    *,
    ...
    ----------
    reg : Signal
        The FSM's state register. Must have a driver that's the next-state
        expression. ``reg.typ`` must equal ``state_cls.typ``.
    state_cls : subclass of State
        The state set. Used to enumerate ``state_value``s over.
    outputs : sequence of Signals
        Moore output signals. Their drivers are evaluated alongside the
        next-state expression.
    max_input_combinations : int
        Defensive cap. Raises ``TooLargeForExhaustiveExtraction`` if the
        product of input domain sizes exceeds this.
    ...
    for s in inputs:
        ...
        if domain > max_input_combinations:
            raise TooLargeForExhaustiveExtraction(
                f"input domain {domain} exceeds cap {max_input_combinations}")
    ...
    for sv in table.state_values:
        ...
        for ic in table.all_input_combos():
            ...
            try:
                ...
                raise ValueError(
                    f"extract_transition_table: failed at state={sv}, inputs={ic}: {e}")
