"""Step 4: walker — find every State Const reachable from a set of root Exprs.

The walker is used for two purposes:

1. **Inventory.** Confirm that a `with` block actually contains references to
   the state class — if it doesn't, the wrapper has nothing to do and exits
   silently.
2. **Input discovery.** While walking, collect every non-state-Const ``Signal``
   leaf. These are the FSM's free inputs whose values the transition-table
   extractor will enumerate.

We do **not** need substitution sites for the encoding-rewrite step:
``State`` Consts are shared object instances (`StateCls.NAME` is one object
referenced everywhere), so mutating ``cls.NAME.value`` in place propagates
to every reference automatically. ``apply_encoding`` in ``_emit.py`` relies
on that property.

The walker reuses the existing ``ExprVisitor`` base class (identity-cached
DFS) so each shared sub-tree is visited only once even when the same wire
...
class _StateConstFinder(ExprVisitor[None]):
    ...
    def visit_signal(self, e: Signal) -> None:
        if e._driver is not None:
            # Auto-shared wires (sig_N) wrap inner Exprs — recurse through.
            # User-named signals (inputs, regs, outputs) usually also have
            # drivers, but their drivers were built in user code and may
            # legitimately contain State Consts the wrapper cares about.
            if id(e) not in self._seen_signal_ids:
                self._seen_signal_ids.add(id(e))
                self.visit(e._driver)
        else:
            # Leaf signal — treat as a free input candidate (drivers attached
            # outside the with-block are still considered inputs from the
...
def walk(roots: Iterable[Expr], state_cls: "type[State]") -> _StateConstFinder:
    """Walk every Expr in `roots`, returning the finder with populated
    ``found_consts`` and ``input_signals`` lists.

    Both lists may contain duplicates by *object identity*; callers that need
    deduplication should run ``{id(x): x for x in lst}.values()`` themselves.
    """
    finder = _StateConstFinder(state_cls)
    for root in roots:
        if root is None:
            continue
        # Treat a bare Const root as if it were a child of a virtual parent —
        # the visitor's visit_const handles the detection.
        finder.visit(root)
    return finder
...
def find_input_signals(
    ...
    *,
    ...
    """Return all non-state Signal leaves reachable from ``roots``, minus any
    listed in ``exclude``. Used by ``extract_transition_table`` to discover
    the FSM's free inputs.

    A ``Signal`` is treated as a leaf when:
      * it has no driver, or
      * it's a user-named ``Signal`` with `kind == "input"` (truly external).

    Auto-shared CSE wires (``_auto_generated=True``) are recursed *through*
    rather than reported, because their drivers are the actual logic.
    """
    finder = walk(roots, state_cls)
