...
the `_ArrayIndex` read leaf, plus the small port records. Kept out of `spirehdl.py` so the
core stays lean: everything synthesis-side (Verilog emission, read-under-write, masks,
registered-read pipelining) lives in `spirehdl/primitives/`. Designs use `MemoryPrimitive` /
...
from spirehdl.signal_name_inference import infer_signal_name_from_assignment
from spirehdl.spirehdl import Bool, Const, Expr, HDLType, Signal, UInt
...
class _MemoryArray(Signal):
    ...
    *emission*, registered-read pipelining, and all `custom_verilog` live in `primitives/`.
    ...
    def step(self, sim) -> None:
        ...
        """
        from spirehdl.spirehdl_simulator import _to_bits  # lazy, avoids cycles
