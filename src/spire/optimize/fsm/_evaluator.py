...
The operator table mirrors ``spire_simplify._fold_op2`` (same width-masked Verilog semantics) so symbolic eval
...
class _Eval(ExprVisitor[int]):
    ...
    def visit_concat(self, e: Concat) -> int:
        # Concat is LSB-first in spire: parts[0] occupies the low bits.
        out = 0
        shift = 0
        for p in e.parts:
